...
def main():
    ...
    for i, batch_dict in enumerate(inference_dataloader):
        ...
        with torch.no_grad():
            ...
            print("Predicted label: ", pred_dicts[0]['pred_labels']) 
            print("Predicted bounding box: \n", pred_dicts[0]['pred_boxes']) 
            print("Predicted score = confidence: ", pred_dicts[0]['pred_scores'])
    ...
    """
    gt_boxes_pose_velo = gt_boxes_pose_cam @ Tr_cam_to_velo 
    ...
    # Apply transformation cam to velo to center of all gt_boxes
    ## create homogenous coordinates for the location of the gt_boxes
    gt_boxes_loc_homogeneous = np.hstack((gt_boxes_loc, np.ones((gt_boxes_loc.shape[0], 1), dtype=np.float32))) # add a column of ones to the location array to have a homogeneous coordinate system

    # create a column vector for the location of the gt_boxes 
    gt_boxes_loc_homogeneous_1 = gt_boxes_loc_homogeneous.reshape(-1, 4, 1)

    
    gt_boxes_loc_velo = Tr_cam_to_velo @ gt_boxes_loc_homogeneous_1 #vstack to add a row of ones to the location array to have a homogeneous coordinate system

    # convert back to row vector for the location of the gt_boxes 
    gt_boxes_loc_velo = gt_boxes_loc_velo.reshape(-1, 4) #reshape to have the same shape as the location array

    # Create gt_boxes in velo cf in format [Nx7] with [x, y, z, l, w, h, ry]
    gt_boxes_in_velodyne_cf = np.zeros((gt_boxes_loc_velo.shape[0], 7)) 
    gt_boxes_in_velodyne_cf[:, :3] = gt_boxes_loc_velo[:, :3]       # location from the first 3 columns of gt_boxes_loc_velo
    gt_boxes_in_velodyne_cf[:, 3:6] = gt_boxes_dim                  # dimensions from the dimensions array
    gt_boxes_in_velodyne_cf[:, -1] = gt_boxes_rots                   # rotation around y-axis from the rotation_y array
